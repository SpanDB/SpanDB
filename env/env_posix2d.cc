...
class Posix2dEnv : public Env {
 public:
  using Env::NewWritableFile;
  explicit Posix2dEnv(std::string prefix_, int max_level, std::string data_dir);
  ...
  Status NewSequentialFile(const std::string& fname,
    ...
    std::string fname_;
    bool s_ = GetPrefixPath(fname, fname_);
    result->reset();
    int fd = -1;
...
#ifdef ROCKSDB_LITE
      return Status::IOError(fname_, "Direct I/O not supported in RocksDB lite");
#endif  // !ROCKSDB_LITE
    ...
    do {
      ...
      fd = open(fname_.c_str(), flags, GetDBFileMode(allow_non_owner_access_));
    } while (fd < 0 && errno == EINTR);
    if (fd < 0) {
      return IOError("While opening a file for sequentially reading", fname_,
                     errno);
    }
    ...
    if (options.use_direct_reads && !options.use_mmap_reads) {
      ...
      if (fcntl(fd, F_NOCACHE, 1) == -1) {
        close(fd);
        return IOError("While fcntl NoCache", fname_, errno);
      }
#endif
    } else {
      do {
      ...
      if (file == nullptr) {
        close(fd);
        return IOError("While opening file for sequentially read", fname_,
                       errno);
      }
    }
    result->reset(new PosixSequentialFile(fname_, file, fd, options));
    return Status::OK();
  }
  ...
  Status NewRandomAccessFile(const std::string& fname,
    ...
    if (fd < 0) {
      return IOError("While open a file for random read", fname_, errno);
    }
    SetFD_CLOEXEC(fd, &options);
    ...
    if (options.use_mmap_reads && sizeof(void*) >= 8) {
      ...
      uint64_t size;
      s = GetFileSize(fname_, &size);
      if (s.ok()) {
        void* base = mmap(nullptr, size, PROT_READ, MAP_SHARED, fd, 0);
        if (base != MAP_FAILED) {
          result->reset(new PosixMmapReadableFile(fd, fname_, base,
                                                  size, options));
        } else {
          s = IOError("while mmap file for read", fname_, errno);
          close(fd);
        }
      }
    } else {
      ...
      result->reset(new PosixRandomAccessFile(fname_, fd, options));
    }
    return s;
  }
  ...
  virtual Status OpenWritableFile(const std::string& fname,
    ...
    std::string fname_;
    bool s_ = GetPrefixPath(fname, fname_);
    //fprintf(stderr, "OpenWritableFile %s:%d: %s \n", __FILE__, __LINE__, fname_.c_str()); 
    result->reset();
    Status s;
    int fd = -1;
    ...
    if (fd < 0) {
      s = IOError("While open a file for appending", fname_, errno);
      return s;
    }
    SetFD_CLOEXEC(fd, &options);
    ...
    if (options.use_mmap_writes) {
      if (!checkedDiskForMmap_) {
        ...
        if (!SupportsFastAllocate(fname_)) {
          forceMmapOff_ = true;
        }
    ...
    if (options.use_mmap_writes && !forceMmapOff_) {
      result->reset(new PosixMmapFile(fname_, fd, page_size_, options));
    } else if (options.use_direct_writes && !options.use_mmap_writes) {
#ifdef OS_MACOSX
      if (fcntl(fd, F_NOCACHE, 1) == -1) {
        close(fd);
        s = IOError("While fcntl NoCache an opened file for appending", fname_,
                    errno);
        return s;
      }
      ...
      if (directio(fd, DIRECTIO_ON) == -1) {
        if (errno != ENOTTY) { // ZFS filesystems don't support DIRECTIO_ON
          close(fd);
          s = IOError("While calling directio()", fname_, errno);
          return s;
        }
      }
#endif
      result->reset(new PosixWritableFile(fname_, fd, options));
    } else {
      // disable mmap writes
      EnvOptions no_mmap_writes_options = options;
      no_mmap_writes_options.use_mmap_writes = false;
      result->reset(new PosixWritableFile(fname_, fd, no_mmap_writes_options));
    }
    return s;
  }
  ...
  Status ReuseWritableFile(const std::string& fname,
                           ...
                           std::unique_ptr<WritableFile>* result,
                           const EnvOptions& options) override {
    std::string fname_, old_fname_;
    bool s1_ = GetPrefixPath(fname, fname_);
    bool s2_ = GetPrefixPath(old_fname, old_fname_);
    ...
    do {
      IOSTATS_TIMER_GUARD(open_nanos);
      fd = open(old_fname_.c_str(), flags,
                GetDBFileMode(allow_non_owner_access_));
    } while (fd < 0 && errno == EINTR);
    if (fd < 0) {
      s = IOError("while reopen file for write", fname_, errno);
      return s;
    }

    SetFD_CLOEXEC(fd, &options);
    // rename into place
    if (rename(old_fname_.c_str(), fname_.c_str()) != 0) {
      s = IOError("while rename file to " + fname_, old_fname_, errno);
      close(fd);
      return s;
    }
    ...
    if (options.use_mmap_writes && !forceMmapOff_) {
      ...
      if (fcntl(fd, F_NOCACHE, 1) == -1) {
        close(fd);
        s = IOError("while fcntl NoCache for reopened file for append", fname_,
                    errno);
        return s;
      }
#elif defined(OS_SOLARIS)
  ...
  Status NewRandomRWFile(const std::string& fname,
    ...
    while (fd < 0) {
      ...
      if (fd < 0) {
        ...
        return IOError("While open file for random read/write", fname_, errno);
      }
    }

    SetFD_CLOEXEC(fd, &options);
    result->reset(new PosixRandomRWFile(fname_, fd, options));
    return Status::OK();
  }
  ...
  Status NewMemoryMappedFileBuffer(
    ...
    while (fd < 0) {
      IOSTATS_TIMER_GUARD(open_nanos);
      fd = open(fname_.c_str(), flags, 0644);
      if (fd < 0) {
        // Error while opening the file
        if (errno == EINTR) {
          continue;
        ...
        status =
            IOError("While open file for raw mmap buffer access", fname_, errno);
        break;
      }
    }
    uint64_t size;
    if (status.ok()) {
      status = GetFileSize(fname_, &size);
    }
    void* base = nullptr;
    if (status.ok()) {
      base = mmap(nullptr, static_cast<size_t>(size), PROT_READ | PROT_WRITE,
                  MAP_SHARED, fd, 0);
      if (base == MAP_FAILED) {
        status = IOError("while mmap file for read", fname_, errno);
      }
    }
    if (status.ok()) {
      result->reset(
  ...
  Status FileExists(const std::string& fname) override {
    ...
    int result = access(fname_.c_str(), F_OK);
    ...
    switch (err) {
      ...
      default:
        ...
        return Status::IOError("Unexpected error(" + ToString(err) +
                               ") accessing file `" + fname_ + "' ");
    }
  }

  Status GetChildren(const std::string& dir,
                     std::vector<std::string>* result) override {
    //fprintf(stderr, "GetChildren %s:%d: %s \n", __FILE__, __LINE__, dir.c_str());      
    result->clear();
  ...
  Status DeleteFile(const std::string& fname) override {
    ...
    if (unlink(fname_.c_str()) != 0) {
      result = IOError("while unlink() file", fname_, errno);
    }
    return result;
  };

  Status CreateDir(const std::string& name) override {
    //fprintf(stderr, "CreateDir %s:%d: %s \n", __FILE__, __LINE__, name.c_str());
    Status result;
  ...
  Status CreateDirIfMissing(const std::string& name) override {
    Status result;
    //fprintf(stderr, "CreateDirIfMissing %s:%d: %s \n", __FILE__, __LINE__, name.c_str());
    //return Status::NotSupported("CreateDirIfMissing not implemented in Posix2dEnv ");
    if (mkdir(name.c_str(), 0755) != 0) {
      if (errno != EEXIST) {
  ...
  Status GetFileSize(const std::string& fname, uint64_t* size) override {
    ...
    Status s;
    struct stat sbuf;
    if (stat(fname_.c_str(), &sbuf) != 0) {
      *size = 0;
      s = IOError("while stat a file for size", fname_, errno);
    } else {
      *size = sbuf.st_size;
    }
    return s;
  ...
  Status GetFileModificationTime(const std::string& fname,
    ...
    bool s_ = GetPrefixPath(fname, fname_);
    struct stat s;
    if (stat(fname_.c_str(), &s) !=0) {
      return IOError("while stat a file for modification time", fname_, errno);
    }
  ...
  Status RenameFile(const std::string& src,
    ...
    bool s1_ = GetPrefixPath(src, src_);
    bool s2_ = GetPrefixPath(target, target_);
    Status result;
    if (rename(src_.c_str(), target_.c_str()) != 0) {
      result = IOError("While renaming a file to " + target_, src_, errno);
    }
    return result;
  }
  ...
  Status LinkFile(const std::string& src, const std::string& target) override {
    ...
    bool s2_ = GetPrefixPath(target, target_);
    Status result;
    if (link(src_.c_str(), target_.c_str()) != 0) {
      if (errno == EXDEV) {
  ...
  Status NumFileLinks(const std::string& fname, uint64_t* count) override {
    ...
    struct stat s;
    if (stat(fname_.c_str(), &s) != 0) {
      return IOError("while stat a file for num file links", fname_, errno);
    }
  ...
  Status AreFilesSame(const std::string& first, const std::string& second,
    ...
    bool s1_ = GetPrefixPath(first, first_);
    bool s2_ = GetPrefixPath(second, second_);
    struct stat statbuf[2];
    if (stat(first_.c_str(), &statbuf[0]) != 0) {
      return IOError("stat file", first, errno);
    }
    if (stat(second_.c_str(), &statbuf[1]) != 0) {
  ...
  Status LockFile(const std::string& fname, FileLock** lock) override {
    ...
    *lock = nullptr;
    ...
    if (lockedFiles.insert(fname_).second == false) {
      mutex_lockedFiles.Unlock();
      errno = ENOLCK;
      return IOError("lock ", fname_, errno);
    }
    ...
    if (fd < 0) {
      result = IOError("while open a file for lock", fname_, errno);
    } else if (LockOrUnlock(fd, true) == -1) {
      // if there is an error in locking, then remove the pathname from lockedfiles
      lockedFiles.erase(fname_);
      result = IOError("While lock file", fname_, errno);
      close(fd);
    } else {
      SetFD_CLOEXEC(fd, nullptr);
      PosixFileLock* my_lock = new PosixFileLock;
      my_lock->fd_ = fd;
      my_lock->filename = fname_;
      *lock = my_lock;
    }
  ...
  Status GetFreeSpace(const std::string& fname, uint64_t* free_space) override {
    ...
    std::string fname_;
    bool s_ = GetPrefixPath(fname, fname_);
    if (statvfs(fname_.c_str(), &sbuf) < 0) {
      return IOError("While doing statvfs", fname_, errno);
    }
...
#endif
      SetFD_CLOEXEC(fd, nullptr);
      result->reset(new PosixLogger(f, &Posix2dEnv::gettid, this));
      return Status::OK();
    }
  }
  ...
  bool GetPrefixPath(const std::string& fname,
                         std::string& output_path)  {
                           
    if(!has_suffix(fname, ".sst")){
      output_path.assign(fname);
      return false;
    }
    if(fname.find(prefix) != std::string::npos){
      output_path.assign(fname);
      return false;
    }
    std::size_t found = fname.find_last_of("/");
    output_path = prefix+"/"+fname.substr(found+1);
    return true;
  }
...
Posix2dEnv::Posix2dEnv(std::string prefix_, int max_level, std::string data_dir)
  ...
  if(max_level) {
    assert(data_dir != "");
    int MAXLEVEL = 20;
    uint64_t moved_size = 0;
    std::vector<std::vector<std::string>> files_level;
    files_level.reserve(MAXLEVEL);
    // 1. get file
    FILE* inputfile = fopen((data_dir + "/spdk.sst").c_str(), "r");
    if (inputfile == nullptr) {
      fprintf(stderr, "Can not open file %s\n",(data_dir + "/spdk.sst").c_str());
      return;
    }
    char line[201];
    bool stop = false;
    ...
    //2. move
    printf("moving data from %s to %s\n", data_dir.c_str(), prefix.c_str());
    for (int i = 0; i <= max_level; i++) {
      if(stop) break;
      printf("moving level %d...\n", i);
      for (auto& file : files_level[i]) {
        if(stop) break;
        std::string fname = data_dir + "/" + file;
        std::string destname = prefix + "/" + file;
        system(("mv " + fname + " " + destname).c_str());
        //add file size
        struct stat sbuf;
        if (stat(destname.c_str(), &sbuf) != 0) {
          fprintf(stderr, "Error when getting %s size", destname.c_str());
        } else {
          moved_size += sbuf.st_size;
        }
        //check left space
        uint64_t free_space = 0;
        GetFreeSpace(destname, &free_space);
        if(free_space < (20ull<<30)){
          stop = true;
          printf("stop moving because of space not enought\n");
        }
      }
    }
    printf("total moved data: %.2lf GB\n", moved_size/1024.0/1024.0/1024.0);
  }
...
void Posix2dEnv::Schedule(void (*function)(void* arg1), void* arg, Priority pri,
                        void* tag, void (*unschedFunction)(void* arg)) {
  assert(pri >= Priority::BOTTOM && pri <= Priority::HIGH);
  thread_pools_[pri].Schedule(function, arg, tag, unschedFunction);
}

int Posix2dEnv::UnSchedule(void* arg, Priority pri) {
  return thread_pools_[pri].UnSchedule(arg);
}

unsigned int Posix2dEnv::GetThreadPoolQueueLen(Priority pri) const {
  assert(pri >= Priority::BOTTOM && pri <= Priority::HIGH);
...
void Posix2dEnv::StartThread(void (*function)(void* arg), void* arg) {
  pthread_t t;
...
// Default Posix Env
//
Env* NewPosix2dEnv(std::string prefix_, int max_level, std::string data_dir) {
  return new Posix2dEnv(prefix_, max_level, data_dir);
}
