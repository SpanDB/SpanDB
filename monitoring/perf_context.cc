...
PerfContext::PerfContext(const PerfContext& other) {
  ...
  ssdlogging_time = other.ssdlogging_time;  // spandb
...
void PerfContext::Reset() {
  ...
  write_wal_time = 0;
  ssdlogging_time = 0; //spandb
  complete_parallel_memtable_time = 0; //spandb
...
std::string PerfContext::ToString(bool exclude_zero_counters) const {
  ...
  PERF_CONTEXT_OUTPUT(ssdlogging_time);  // spandb
