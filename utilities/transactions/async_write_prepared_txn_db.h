...
#include "utilities/transactions/async_write_prepared_txn.h"
...
class AsyncWritePreparedTxnDB : public PessimisticTransactionDB {
 public:
  explicit AsyncWritePreparedTxnDB(DB* db,
    ...
    Init(txn_db_options);
    AsyncStart();  // add for async
  }

  explicit AsyncWritePreparedTxnDB(StackableDB* db,
                              const TransactionDBOptions& txn_db_options)
      : PessimisticTransactionDB(db, txn_db_options),
        SNAPSHOT_CACHE_BITS(txn_db_options.wp_snapshot_cache_bits),
        SNAPSHOT_CACHE_SIZE(static_cast<size_t>(1ull << SNAPSHOT_CACHE_BITS)),
  ...
  virtual ~AsyncWritePreparedTxnDB();
  ...
  Status WriteInternal(const WriteOptions& write_options, WriteBatch* batch,
                       size_t batch_cnt, AsyncWritePreparedTxn* txn);

  //add for async
  Status ProcessBeforeLogging(const WriteOptions& write_options_orig, WriteBatch* batch,
                       size_t batch_cnt, AsyncWritePreparedTxn* txn, uint64_t *seq_used);

  Status ProcessAfterLogging(const WriteOptions& write_options_orig, WriteBatch* batch,
                      size_t batch_cnt, AsyncWritePreparedTxn* txn, uint64_t seq_used);
  ...
  inline bool IsInSnapshot(uint64_t prep_seq, uint64_t snapshot_seq,
    ...
    do {
      ...
      TEST_SYNC_POINT(
          "AsyncWritePreparedTxnDB::IsInSnapshot:max_evicted_seq_:pause");
      TEST_SYNC_POINT(
          "AsyncWritePreparedTxnDB::IsInSnapshot:max_evicted_seq_:resume");
      was_empty = delayed_prepared_empty_.load(std::memory_order_acquire);
      TEST_SYNC_POINT(
          "AsyncWritePreparedTxnDB::IsInSnapshot:delayed_prepared_empty_:pause");
      TEST_SYNC_POINT(
          "AsyncWritePreparedTxnDB::IsInSnapshot:delayed_prepared_empty_:resume");
      CommitEntry cached;
      bool exist = GetCommitEntry(indexed_seq, &dont_care, &cached);
      TEST_SYNC_POINT("AsyncWritePreparedTxnDB::IsInSnapshot:GetCommitEntry:pause");
      TEST_SYNC_POINT("AsyncWritePreparedTxnDB::IsInSnapshot:GetCommitEntry:resume");
      ...
      TEST_SYNC_POINT("AsyncWritePreparedTxnDB::IsInSnapshot:prepared_mutex_:pause");
      TEST_SYNC_POINT(
          "AsyncWritePreparedTxnDB::IsInSnapshot:prepared_mutex_:resume");
      if (!was_empty) {
 ...
 private:
    ...
    DisruptorQueue<AsyncWritePreparedTxn *> *submission_queue;
    DisruptorQueue<AsyncWritePreparedTxn::AsyncReadRequest *> *read_queue;
    Status Enqueue(AsyncWritePreparedTxn *txn);
    Status EnqueueRead(AsyncWritePreparedTxn::AsyncReadRequest *req);
    Status HandleAsyncRead(AsyncWritePreparedTxn::AsyncReadRequest *req);
    void ReadReqWorker(int coreid);
    void Stop();
    void BeforeWorker(int coreid);
    void AfterWorker(int coreid);
    void BeforeAndAfterWorker(int coreid);
    void AsyncStart();

    ssdlogging::statistics::MyStat before_time;
    ssdlogging::statistics::MyStat after_time;
    ssdlogging::statistics::MyStat before_wait_time;
    ssdlogging::statistics::MyStat after_wait_time;
    double proLog_time = 1;
    double epiLog_time = 1;
    double read_time = 1;

    std::vector<std::thread> before_threads;
    std::vector<std::thread> after_threads;
    std::vector<std::thread> read_threads;
    std::vector<std::thread> process_threads;
    std::atomic<bool> stop_;

  
  friend class AsyncAddPreparedCallback;
  ...
  friend class AsyncWritePreparedCommitEntryPreReleaseCallback;
  ...
  // internal to AsyncWritePreparedTxnDB
...
class AsyncWritePreparedTxnReadCallback : public ReadCallback {
 public:
  AsyncWritePreparedTxnReadCallback(AsyncWritePreparedTxnDB* db, SequenceNumber snapshot)
  ...
  virtual ~AsyncWritePreparedTxnReadCallback() {
...
class AsyncAddPreparedCallback : public PreReleaseCallback {
 public:
  AsyncAddPreparedCallback(AsyncWritePreparedTxnDB* db, DBImpl* db_impl,
  ...
  virtual Status Callback(SequenceNumber prepare_seq,
    ...
    TEST_SYNC_POINT("AsyncAddPreparedCallback::AddPrepared::begin:pause");
    TEST_SYNC_POINT("AsyncAddPreparedCallback::AddPrepared::begin:resume");
    if (do_lock) {
    ...
    TEST_SYNC_POINT("AsyncAddPreparedCallback::AddPrepared::end");
...
class AsyncWritePreparedCommitEntryPreReleaseCallback : public PreReleaseCallback {
 public:
  ...
  AsyncWritePreparedCommitEntryPreReleaseCallback(
      AsyncWritePreparedTxnDB* db, DBImpl* db_impl, SequenceNumber prep_seq,
      ...
      : db_(db),
        ...
        includes_aux_batch_(aux_batch_cnt > 0) {
          if(!(prep_batch_cnt_ > 0) != (prep_seq == kMaxSequenceNumber))
...
class AsyncWritePreparedRollbackPreReleaseCallback : public PreReleaseCallback {
 public:
  AsyncWritePreparedRollbackPreReleaseCallback(AsyncWritePreparedTxnDB* db,
                                          DBImpl* db_impl,
...
SnapshotBackup AsyncWritePreparedTxnDB::AssignMinMaxSeqs(const Snapshot* snapshot,
                                                    SequenceNumber* min,
                                                    SequenceNumber* max) {
  if (snapshot != nullptr) {
...
bool AsyncWritePreparedTxnDB::ValidateSnapshot(
    const SequenceNumber snap_seq, const SnapshotBackup backed_by_snapshot,
