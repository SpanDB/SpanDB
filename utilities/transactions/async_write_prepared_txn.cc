...
AsyncWritePreparedTxn::AsyncWritePreparedTxn(AsyncWritePreparedTxnDB* txn_db,
  ...
  // would skip overridden functions in AsyncWritePreparedTxn since they are not
  // defined yet in the constructor of PessimisticTransaction
  Initialize(txn_options);
}

bool AsyncWritePreparedTxn::IsFinished(){
  return is_finished.load();
}

Status AsyncWritePreparedTxn::ProcessBeforeLogging(){
  Status s;
	...
	if (commit_without_prepare) {
      ...
      wpt_db_->ProcessBeforeLogging(write_options_, 
                                    GetWriteBatch()->GetWriteBatch(), 
                                    batch_cnt, this, &seq_used_);
...
Status AsyncWritePreparedTxn::ProcessAfterLogging(){

  const size_t batch_cnt = GetWriteBatch()->SubBatchCnt();
  Status s = wpt_db_->ProcessAfterLogging(write_options_, 
                                    GetWriteBatch()->GetWriteBatch(), 
                                    batch_cnt, this, seq_used_);
	if (!name_.empty()) {
...
Status AsyncWritePreparedTxn::AsyncCommit() {
  assert(!is_finished);
  return wpt_db_->Enqueue(this);

  // Status s = ProcessBeforeLogging();
  // assert(s.ok());
  // s = ProcessAfterLogging();
  // return s;
}

Status AsyncWritePreparedTxn::AsyncGetV2(const ReadOptions& options,
                     ColumnFamilyHandle* column_family, const std::string key,
                     std::string* value, bool *finished){
  AsyncReadRequest *req = new AsyncReadRequest(this, finished, options, 
                                             column_family, key, value);
  return wpt_db_->EnqueueRead(req);
}

Status AsyncWritePreparedTxn::AsyncGetV2(const ReadOptions& options, const std::string key,
                     std::string* value, bool *finished) {  
  return AsyncGetV2(options, db_->DefaultColumnFamily(), key, value, finished);
};

Status AsyncWritePreparedTxn::Commit() {
  
  is_finished.store(true); // add for async
  return Status::NotSupported(
        "please use AsyncCommit() in AsyncWritePreparedTxn");
...
void AsyncWritePreparedTxn::Initialize(const TransactionOptions& txn_options) {
  PessimisticTransaction::Initialize(txn_options);
  prepare_batch_cnt_ = 0;
  is_finished.store(false); // add for async
}

void AsyncWritePreparedTxn::MultiGet(const ReadOptions& options,
  ...
  AsyncWritePreparedTxnReadCallback callback(wpt_db_, snap_seq, min_uncommitted,
                                        backed_by_snapshot);
...
Status AsyncWritePreparedTxn::Get(const ReadOptions& options,
                             ColumnFamilyHandle* column_family,
...
Iterator* AsyncWritePreparedTxn::GetIterator(const ReadOptions& options) {
...
Status AsyncWritePreparedTxn::PrepareInternal() {
...
Status AsyncWritePreparedTxn::CommitWithoutPrepareInternal() {
  // For each duplicate key we account for a new sub-batch
  const size_t batch_cnt = GetWriteBatch()->SubBatchCnt();
  return CommitBatchInternal(GetWriteBatch()->GetWriteBatch(), batch_cnt);
}

Status AsyncWritePreparedTxn::CommitBatchInternal(WriteBatch* batch,
                                             size_t batch_cnt) {
  return wpt_db_->WriteInternal(write_options_, batch, batch_cnt, this);
}

Status AsyncWritePreparedTxn::CommitInternal() {
  ...
  AsyncWritePreparedCommitEntryPreReleaseCallback update_commit_map_with_aux_batch(
...
Status AsyncWritePreparedTxn::RollbackInternal() {
  ROCKS_LOG_WARN(db_impl_->immutable_db_options().info_log,
  ...
  struct RollbackWriteBatchBuilder : public WriteBatch::Handler {
    ...
    RollbackWriteBatchBuilder(
        DBImpl* db, AsyncWritePreparedTxnDB* wpt_db, SequenceNumber snap_seq,
  ...
  AsyncWritePreparedRollbackPreReleaseCallback update_commit_map_with_prepare(
...
Status AsyncWritePreparedTxn::ValidateSnapshot(ColumnFamilyHandle* column_family,
                                          const Slice& key,
  ...
  *tracked_at_seq = snap_seq;
  ...
  AsyncWritePreparedTxnReadCallback snap_checker(wpt_db_, snap_seq, min_uncommitted,
                                            kBackedByDBSnapshot);
...
void AsyncWritePreparedTxn::SetSnapshot() {
  const bool kForWWConflictCheck = true;
  SnapshotImpl* snapshot = wpt_db_->GetSnapshotInternal(kForWWConflictCheck);
  SetSnapshotInternal(snapshot);
}

Status AsyncWritePreparedTxn::RebuildFromWriteBatch(WriteBatch* src_batch) {
