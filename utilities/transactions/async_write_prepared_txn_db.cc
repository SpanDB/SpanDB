...
#include "utilities/transactions/async_write_prepared_txn_db.h"

namespace rocksdb {

Status AsyncWritePreparedTxnDB::Initialize(
 ...
 // db_impl_->SetSnapshotChecker(new WritePreparedSnapshotChecker(this)); // need modification for async
    db_impl_->SetSnapshotChecker(new AsyncWritePreparedSnapshotChecker(this)); // modify for async

  // A callback to commit a single sub-batch
  class CommitSubBatchPreReleaseCallback : public PreReleaseCallback {
   public:
    explicit CommitSubBatchPreReleaseCallback(AsyncWritePreparedTxnDB* db)
        : db_(db) {}
...
//add for async
void AsyncWritePreparedTxnDB::AsyncStart(){
  stop_.store(false);
  submission_queue = new DisruptorQueue<AsyncWritePreparedTxn *>();
  read_queue = new DisruptorQueue<AsyncWritePreparedTxn::AsyncReadRequest *>();
  ...
  /*auto fn1 = std::bind(&AsyncWritePreparedTxnDB::BeforeWorker, this, std::placeholders::_1);
  for(int i=0; i<n; i++){
    before_threads.emplace_back(fn1, numCores-1-logs-i);
  }
  auto fn2 = std::bind(&AsyncWritePreparedTxnDB::AfterWorker, this, std::placeholders::_1);
  for(int i=0; i<m; i++){
    after_threads.emplace_back(fn2, numCores-1-logs-n-i);
  }*/
  ...
  auto fn2 = std::bind(&AsyncWritePreparedTxnDB::AfterWorker, this, std::placeholders::_1);
  for(int i=0; i<m1; i++){
    after_threads.emplace_back(fn2, numCores-1-logs-n1-i);
  }
  auto fn3 = std::bind(&AsyncWritePreparedTxnDB::ReadReqWorker, this, std::placeholders::_1);
  for(int i=0; i<r; i++){
    read_threads.emplace_back(fn3, numCores-1-logs-n1-m1-i);
  }
  auto fn = std::bind(&AsyncWritePreparedTxnDB::BeforeAndAfterWorker, this, std::placeholders::_1);
  for(int i=0; i<n+m-n1-m1; i++){
    process_threads.emplace_back(fn, numCores-1-logs-n1-m1-r-i);
  }
}

void AsyncWritePreparedTxnDB::Stop(){
    db_impl_->StopLogging();
    stop_.store(true);
    if(submission_queue != nullptr)
      submission_queue->stop();
    if(read_queue != nullptr)
      read_queue->stop();
    for(auto &t : before_threads)
      t.join();
    for(auto &t : after_threads)
      t.join();
    for(auto &t : read_threads)
      t.join();
    for(auto &t : process_threads)
      t.join();
    before_time.print_data("time before");
    after_time.print_data("time after");
    before_wait_time.print_data("Before wait time");
    after_wait_time.print_data("After wait time");
}

Status AsyncWritePreparedTxnDB::Enqueue(AsyncWritePreparedTxn *txn){
  submission_queue->WriteInBuf(txn);
  return Status::OK();
}

Status AsyncWritePreparedTxnDB::EnqueueRead(AsyncWritePreparedTxn::AsyncReadRequest *req){
  //printf("enqueue %ld: key: %ld, value: %ld\n", (uint64_t)req, (uint64_t)&(req->key_), (uint64_t)req->value_);
  read_queue->WriteInBuf(req);
  return Status::OK();
}

Status AsyncWritePreparedTxnDB::HandleAsyncRead(AsyncWritePreparedTxn::AsyncReadRequest *req){
  assert(req->value_ != nullptr);
  PinnableSlice pinnable_val(req->value_);
  assert(!pinnable_val.IsPinned());
  //printf("handle1 %ld: key: %ld, value: %ld\n", (uint64_t)req, (uint64_t)&req->key_, (uint64_t)req->value_);
  auto s = req->txn_->Get(req->options_, req->column_family_, req->key_, &pinnable_val);
  //printf("handle2 %ld: key: %ld, value: %ld\n", (uint64_t)req, (uint64_t)&req->key_, (uint64_t)req->value_);
  if (s.ok() && pinnable_val.IsPinned()) {
    req->value_->assign(pinnable_val.data(), pinnable_val.size());
  }
  //printf("handle3 %ld: key: %ld, value: %ld\n", (uint64_t)req, (uint64_t)&req->key_, (uint64_t)req->value_);
  //printf("get %ld: key: %ld, value:  %ld\n", (uint64_t)req, (uint64_t)&req->key_,  (uint64_t)req->value_);
  *(req->finished_) = true;
  return s;
}

void AsyncWritePreparedTxnDB::ReadReqWorker(int coreid){
  ssdlogging::SetAffinity("SpanDB reader", coreid);
  while(true){
    if(UNLIKELY(stop_.load()))
      break;
    const int64_t seq = read_queue->GetReadableSeq();
    if(seq < 0)
			continue;
    AsyncWritePreparedTxn::AsyncReadRequest *req = read_queue->ReadFromBuf(seq);
    assert(req!=nullptr);
    //printf("req: %ld\n", (uint64_t)req);
    read_queue->FinishReading(seq);
    HandleAsyncRead(req);
    delete req;
  }
}

void AsyncWritePreparedTxnDB::BeforeWorker(int coreid){
  ssdlogging::SetAffinity("SpanDB Prolog", coreid);
  while(true){
    if(UNLIKELY(stop_.load()))
      break;
    const int64_t seq = submission_queue->GetReadableSeq();
    if(seq < 0)
			continue;
    AsyncWritePreparedTxn *txn = submission_queue->ReadFromBuf(seq);
    assert(txn!=nullptr);
    //before_wait_time.add(SPDK_TIME_DURATION(txn->start_time, SPDK_TIME, spdk_tsc_rate));
    submission_queue->FinishReading(seq);
    //auto start = SPDK_TIME;
    txn->ProcessBeforeLogging();
    //before_time.add(SPDK_TIME_DURATION(start, SPDK_TIME, spdk_tsc_rate));
  }
}

void AsyncWritePreparedTxnDB::AfterWorker(int coreid){
  ssdlogging::SetAffinity("SpanDB Epilog", coreid);
  while(true){
    if(UNLIKELY(stop_.load()))
      break;
    void *ptr = static_cast<AsyncWritePreparedTxn *>(db_impl_->GetFromLogging());
    if(UNLIKELY(ptr == nullptr))
      continue;
    AsyncWritePreparedTxn *txn = static_cast<AsyncWritePreparedTxn *>(ptr);
    //after_wait_time.add(SPDK_TIME_DURATION(txn->finish_logging, SPDK_TIME, spdk_tsc_rate));
    //auto start = SPDK_TIME;
    txn->ProcessAfterLogging();
    //after_wait_time.add(SPDK_TIME_DURATION(start, SPDK_TIME, spdk_tsc_rate));
    //time.add(SPDK_TIME_DURATION(txn->start_time, SPDK_TIME, spdk_tsc_rate));
    //time.add(SPDK_TIME_DURATION(start, SPDK_TIME, spdk_tsc_rate));
  } 
}

void AsyncWritePreparedTxnDB::BeforeAndAfterWorker(int coreid){
  ssdlogging::SetAffinity("SpanDB worker", coreid);
  while(true){
    if(UNLIKELY(stop_.load()))
      break;
    int n1 = submission_queue->Length();
    int n2 = db_impl_->LoggingQueueLength();
    if(n1 == 0 && n2 == 0)
      continue;
    double p = (Random::GetTLSInstance()->Next() & ((1<<20)-1))*1.0/(1<<20);
    if(p < (n1*1.0)/(n1 + 3*n2)){  // ProLog
      const int64_t seq = submission_queue->GetReadableSeq();
      if(seq < 0)
        continue;
      AsyncWritePreparedTxn *txn = submission_queue->ReadFromBuf(seq);
...
Status AsyncWritePreparedTxnDB::ProcessBeforeLogging(const WriteOptions& write_options_orig, WriteBatch* batch,
                      size_t batch_cnt, AsyncWritePreparedTxn* txn, uint64_t *seq_used){
  ...
  *seq_used = kMaxSequenceNumber;
  ...
  AsyncAddPreparedCallback add_prepared_callback(
      this, db_impl_, batch_cnt,
      db_impl_->immutable_db_options().two_write_queues,
      !kSeperatePrepareCommitBatches);
  AsyncWritePreparedCommitEntryPreReleaseCallback update_commit_map(
  ...
  auto s = db_impl_->WriteImplBeforeLogging(write_options, batch, nullptr, nullptr,
...
Status AsyncWritePreparedTxnDB::ProcessAfterLogging(const WriteOptions& write_options_orig, WriteBatch* batch,
                      size_t batch_cnt, AsyncWritePreparedTxn* txn, uint64_t seq_used){
  if (batch_cnt == 0) {  // not provided, then compute it
    // TODO(myabandeh): add an option to allow user skipping this cost
    SubBatchCounter counter(*GetCFComparatorMap());
  ...
  Status s = db_impl_->UnorderedWriteMemtable(write_options, batch, nullptr,
                                      no_log_ref, seq_used, sub_batch_cnt);
  ...
  AsyncWritePreparedCommitEntryPreReleaseCallback update_commit_map_with_prepare(
  ...
  s = db_impl_->WriteImplBeforeLogging(write_options, &empty_batch, nullptr, nullptr,
...
Status AsyncWritePreparedTxnDB::VerifyCFOptions(
...
Transaction* AsyncWritePreparedTxnDB::BeginTransaction(
  ...
  if (old_txn != nullptr) {
    ...
    return new AsyncWritePreparedTxn(this, write_options, txn_options);
  }
}

Status AsyncWritePreparedTxnDB::Write(const WriteOptions& opts,
                                 WriteBatch* updates) {
  if (txn_db_options_.skip_concurrency_control) {
    // Skip locking the rows
    const size_t UNKNOWN_BATCH_CNT = 0;
    AsyncWritePreparedTxn* NO_TXN = nullptr;
...
Status AsyncWritePreparedTxnDB::Write(
...
Status AsyncWritePreparedTxnDB::WriteInternal(const WriteOptions& write_options_orig,
                                         WriteBatch* batch, size_t batch_cnt,
                                         AsyncWritePreparedTxn* txn) {
  ROCKS_LOG_DETAILS(db_impl_->immutable_db_options().info_log,
                    "CommitBatchInternal");
  if (batch->Count() == 0) {
    // Otherwise our 1 seq per batch logic will break since there is no seq
...
Status AsyncWritePreparedTxnDB::Get(const ReadOptions& options,
                               ColumnFamilyHandle* column_family,
                               const Slice& key, PinnableSlice* value) {
  SequenceNumber min_uncommitted, snap_seq;
  const SnapshotBackup backed_by_snapshot =
      AssignMinMaxSeqs(options.snapshot, &min_uncommitted, &snap_seq);
  AsyncWritePreparedTxnReadCallback callback(this, snap_seq, min_uncommitted,
                                        backed_by_snapshot);
...
void AsyncWritePreparedTxnDB::UpdateCFComparatorMap(
    const std::vector<ColumnFamilyHandle*>& handles) {
...
void AsyncWritePreparedTxnDB::UpdateCFComparatorMap(ColumnFamilyHandle* h) {
...
std::vector<Status> AsyncWritePreparedTxnDB::MultiGet(
...
struct AsyncWritePreparedTxnDB::IteratorState {
  IteratorState(AsyncWritePreparedTxnDB* txn_db, SequenceNumber sequence,
                std::shared_ptr<ManagedSnapshot> s,
                SequenceNumber min_uncommitted)
      : callback(txn_db, sequence, min_uncommitted, kBackedByDBSnapshot),
        snapshot(s) {}

  AsyncWritePreparedTxnReadCallback callback;
  std::shared_ptr<ManagedSnapshot> snapshot;
};

namespace {
static void CleanupWritePreparedTxnDBIterator(void* arg1, void* /*arg2*/) {
  delete reinterpret_cast<AsyncWritePreparedTxnDB::IteratorState*>(arg1);
}
}  // anonymous namespace

Iterator* AsyncWritePreparedTxnDB::NewIterator(const ReadOptions& options,
                                          ColumnFamilyHandle* column_family) {
...
Status AsyncWritePreparedTxnDB::NewIterators(
    const ReadOptions& options,
...
void AsyncWritePreparedTxnDB::Init(const TransactionDBOptions& /* unused */) {
...
void AsyncWritePreparedTxnDB::CheckPreparedAgainstMax(SequenceNumber new_max,
                                                 bool locked) {
...
void AsyncWritePreparedTxnDB::AddPrepared(uint64_t seq, bool locked) {
...
void AsyncWritePreparedTxnDB::AddCommitted(uint64_t prepare_seq, uint64_t commit_seq,
                                      uint8_t loop_cnt) {
  ROCKS_LOG_DETAILS(info_log_, "Txn %" PRIu64 " Committing with %" PRIu64,
                    prepare_seq, commit_seq);
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::AddCommitted:start");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::AddCommitted:start:pause");
  ...
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::AddCommitted:end");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::AddCommitted:end:pause");
}

void AsyncWritePreparedTxnDB::RemovePrepared(const uint64_t prepare_seq,
                                        const size_t batch_cnt) {
  TEST_SYNC_POINT_CALLBACK(
      "RemovePrepared:Start",
      const_cast<void*>(reinterpret_cast<const void*>(&prepare_seq)));
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::RemovePrepared:pause");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::RemovePrepared:resume");
  ROCKS_LOG_DETAILS(info_log_,
...
bool AsyncWritePreparedTxnDB::GetCommitEntry(const uint64_t indexed_seq,
...
bool AsyncWritePreparedTxnDB::AddCommitEntry(const uint64_t indexed_seq,
...
bool AsyncWritePreparedTxnDB::ExchangeCommitEntry(const uint64_t indexed_seq,
...
void AsyncWritePreparedTxnDB::AdvanceMaxEvictedSeq(const SequenceNumber& prev_max,
...
const Snapshot* AsyncWritePreparedTxnDB::GetSnapshot() {
  const bool kForWWConflictCheck = true;
  return GetSnapshotInternal(!kForWWConflictCheck);
}

SnapshotImpl* AsyncWritePreparedTxnDB::GetSnapshotInternal(
  ...
  auto min_uncommitted = AsyncWritePreparedTxnDB::SmallestUnCommittedSeq();
  SnapshotImpl* snap_impl = db_impl_->GetSnapshotImpl(for_ww_conflict_check);
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::GetSnapshotInternal:first");
  assert(snap_impl);
  ...
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::GetSnapshotInternal:end");
  return snap_impl;
}

void AsyncWritePreparedTxnDB::AdvanceSeqByOne() {
...
const std::vector<SequenceNumber> AsyncWritePreparedTxnDB::GetSnapshotListFromDB(
    SequenceNumber max) {
...
void AsyncWritePreparedTxnDB::ReleaseSnapshotInternal(
...
void AsyncWritePreparedTxnDB::CleanupReleasedSnapshots(
...
void AsyncWritePreparedTxnDB::UpdateSnapshots(
    const std::vector<SequenceNumber>& snapshots,
    const SequenceNumber& version) {
  ROCKS_LOG_DETAILS(info_log_, "UpdateSnapshots with version %" PRIu64,
                    version);
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::UpdateSnapshots:p:start");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::UpdateSnapshots:s:start");
#ifndef NDEBUG
  size_t sync_i = 0;
#endif
  ...
  for (; it != snapshots.end() && i < SNAPSHOT_CACHE_SIZE; ++it, ++i) {
    ...
    TEST_IDX_SYNC_POINT("AsyncWritePreparedTxnDB::UpdateSnapshots:p:", ++sync_i);
    TEST_IDX_SYNC_POINT("AsyncWritePreparedTxnDB::UpdateSnapshots:s:", sync_i);
  }
#ifndef NDEBUG
  ...
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::UpdateSnapshots:p:end");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::UpdateSnapshots:s:end");
}

void AsyncWritePreparedTxnDB::CheckAgainstSnapshots(const CommitEntry& evicted) {
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:p:start");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:s:start");
#ifndef NDEBUG
  size_t sync_i = 0;
#endif
  ...
  for (; 0 < ip1; ip1--) {
    ...
    TEST_IDX_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:p:",
                        ++sync_i);
    TEST_IDX_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:s:", sync_i);
  ...
  for (++sync_i; sync_i <= 10; ++sync_i) {
    TEST_IDX_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:p:", sync_i);
    TEST_IDX_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:s:", sync_i);
  }
#endif
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:p:end");
  TEST_SYNC_POINT("AsyncWritePreparedTxnDB::CheckAgainstSnapshots:s:end");
...
bool AsyncWritePreparedTxnDB::MaybeUpdateOldCommitMap(
...
AsyncWritePreparedTxnDB::~AsyncWritePreparedTxnDB() {
  // At this point there could be running compaction/flush holding a
  // SnapshotChecker, which holds a pointer back to AsyncWritePreparedTxnDB.
  // Make sure those jobs finished before destructing AsyncWritePreparedTxnDB.
